#!/usr/bin/env python3
"""
Idempotent manager for db-guard entries in ~/.claude/settings.json.

Usage:
  python3 manage-settings.py install    # Merge hook (safe to re-run)
  python3 manage-settings.py uninstall  # Remove db-guard entries only
"""
...
HOOK_SCRIPT = str(Path.home() / ".claude" / "scripts" / "db-guard-hook.zsh")
...
def install():
    ...
    if _hook_already_registered(pre_tool_use):
        ...
        pre_tool_use.append(DB_GUARD_HOOK_ENTRY)
...
def uninstall():
    ...
    if len(hooks["PreToolUse"]) < before:
        print("✓ Removed db-guard PreToolUse hook")
        changed = True
    ...
    if changed:
        ...
        print("No db-guard entries found in settings.json")
